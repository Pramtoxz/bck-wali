...
**Status Types:**
- `present`: Hadir (hijau) - check in & out normal
...
### 7. Attendance Recap Controller Example
```php
namespace App\Http\Controllers\Api;
...
class AttendanceRecapController extends Controller
{
    public function getMonthlyRecap(Request $request)
    {
        ...
        while ($current <= $endDate) {
            ...
            if ($isFieldDuty) {
                ...
                $isLate = Carbon::parse($attendance->check_in_time)->format('H:i:s') > '08:00:00';
    ...
    public function getStatistics(Request $request)
    {
        ...
        $lateDays = $attendances->filter(function ($attendance) {
            return Carbon::parse($attendance->check_in_time)->format('H:i:s') > '08:00:00';
        })->count();
        ...
        $absentDays = $workingDays - $presentDays - $fieldDutyDays - $leaveDays;
